...
if (!empty($_POST['reset'])) {
	if (!empty($_POST['email'])) {
		...
		if ($member) {
			...
			$url = 'https://procir-study.site/Yamaguchi418/01_main/19/reset_form.php?pass_reset=' . $pass_reset;
			$members = $db->prepare('UPDATE members SET reset_pass = ?, reset_time = NOW() WHERE email = ?');
			$members->execute(array($pass_reset, $email));
			mb_language('Japanese');
			mb_internal_encoding('utf-8');
			$to = $email;
			$subject = '[プロサー掲示板]ログインパスワード変更';
			$message = '下記サイトにアクセスしてパスワードの変更手続きをしてください。' . "\r\n" . $url . "\r\n" . '(パスワードの変更期限は30分以内となります。)';
			$headers = 'From: ky02161988@outlook.jp' . "\r\n";
			mb_send_mail($to, $subject, $message, $headers);
		}
		$result = '登録済みのメールアドレスにパスワード再設定用のURLをお送りしました。' . '<br>' . '30分以内に変更手続きをお願いいたします';
	} else {
